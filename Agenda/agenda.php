<div>
    ...
    if (strpos($_POST['nombre'],'<') === false) {
      //comprobar si hay algo introducido
        ...
        if (strpos($_POST['email'],'<') === false) {
          ...
          if (strlen($_POST['email']) > 0) {
            ...
            if (strpos($_POST['email'],'@')) {
              // todo correcto guarda
              $array_agenda[$_POST['nombre']] = $_POST['email'];
            }else {
              echo "<p style='color:red;'>DEBES METER UN EMAIL VÁLIDO(NO CONTEINE @)</p>";
            }
          }else {
            // borra
          ...
          echo "<p style='color:red;'>DEBES METER UN EMAIL VÁLIDO(insercción html)</p>";
        }
      }
    }else {
      echo "<p style='color:red;'>DEBES METER UN NOMBRE VÁLIDO(insercción html)</p>";
    }
  }
	?>
	...
	<table>
	  <tr>
        ...
        $claves = array_keys($array_agenda);

        for ($i=0; $i < 1 ; $i++) {
          echo "<tr>";
          for ($j=1; $j < count($claves) ; $j++) {
            echo "<td><div>".$claves[$j]."</div></td>";
            echo "<td><div>".$array_agenda[$claves[$j]]."</div></td>";
            echo "</tr>";
          }
        }
		  ?>
